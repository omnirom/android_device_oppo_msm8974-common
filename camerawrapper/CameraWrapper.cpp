...
static int camera_get_camera_info(int camera_id, struct camera_info *info)
{
    ...
    if (check_vendor_module())
        return 0;
    int rc = gVendorModule->get_camera_info(camera_id, info);
    // maxwen: the mount angle of the find7op sensor is actually 270
    // but that value should not be used in user-space
    if (camera_id == 0 && info != NULL)
        info->orientation = 90;
    return rc;
}
