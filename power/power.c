...
#define PROFILE_MAX_CPU_TAG "maxCpu"
#define PROFILE_MAX_FREQ_TAG "maxFreq"
...
#ifdef DEBUG
    ALOGI("write (max) %s -> %s", MAX_CPU_PATH, buf);
#endif
...
static void apply_profile(char* profile)
{
    ...
    while(token != NULL) {
        ...
        ALOGI("token %s", token);
#endif
        if (!strcmp(token, PROFILE_MAX_CPU_TAG)) {
            token = strtok(NULL, separator);
            strcpy(max_cpu_profile, token);
        } else if (!strcmp(token, PROFILE_MAX_FREQ_TAG)) {
            token = strtok(NULL, separator);
